...
`Node`s can be hierarchically arranged where `wom` serves as root `Node`. Every attached `Node` inherits the geometrical frame if its parent and have derived transformation.
...
* `options` Object (optional)
  ...
  * `position` Object {x, y, z} (optional) - The `Node`'s position in 3d scene. Default is `{x: 0.0, y: 0.0, z: 0.0}`.
...
* `element` `Node` | `Node[]` Children to create and add to this `Node` instance. _If any element have further children they are getting created and attached recursively._
...
Removes a child `Node` of this `Node` instance. The child will be removed from `Node` hierarchy and every entity belongs to it will be destroyed in the 3d scene. _Removes and destroys children of child recursively._
